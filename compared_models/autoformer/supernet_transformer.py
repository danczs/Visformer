...
def autoformer_b():
    model =  Vision_TransformerSuper(img_size=224,
                patch_size=16,
                embed_dim=640, depth=16,
                ...
                change_qkv=True, abs_pos=True).cuda()

    update_config_from_file('compared_models/autoformer/AutoFormer-B.yaml')
...
def autoformer_s():
    model = Vision_TransformerSuper(img_size=224,
                ...
                relative_position=True,
                change_qkv=True, abs_pos=True).cuda()
    update_config_from_file('compared_models/autoformer/AutoFormer-S.yaml')
    retrain_config = {'layer_num': cfg.RETRAIN.DEPTH, 'embed_dim': [cfg.RETRAIN.EMBED_DIM] * cfg.RETRAIN.DEPTH,
                      'num_heads': cfg.RETRAIN.NUM_HEADS, 'mlp_ratio': cfg.RETRAIN.MLP_RATIO}
    model.set_sample_config(config=retrain_config)
    return model
...
def autoformer_t():
    model =  Vision_TransformerSuper(img_size=224,
                ...
                relative_position=True,
                change_qkv=True, abs_pos=True).cuda()
    update_config_from_file('compared_models/autoformer/AutoFormer-T.yaml')
    retrain_config = {'layer_num': cfg.RETRAIN.DEPTH, 'embed_dim': [cfg.RETRAIN.EMBED_DIM] * cfg.RETRAIN.DEPTH,
                      'num_heads': cfg.RETRAIN.NUM_HEADS, 'mlp_ratio': cfg.RETRAIN.MLP_RATIO}
    model.set_sample_config(config=retrain_config)
    return model
